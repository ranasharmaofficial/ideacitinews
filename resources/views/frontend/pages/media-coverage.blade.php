...
@section('title') Media Coverage @endsection
					...
					@foreach($blog_list as $item)
						<article class="postbox__item format-image mb-60 transition-3">
							<div class="postbox__thumb w-img">
							   <a href="#">
								  <img src="{{ static_asset($item->blog_image) }}" alt="">
							   </a>
							   <div class="postbox__tag">
								  <span>{{ date('d', strtotime($item->created_at)) }}<br>{{ date('Y', strtotime($item->created_at)) }}</span>
							   </div>
							</div>
							<div class="postbox__content">
							   <div class="postbox__meta">
								  <span><a href="#"><i class="far fa-calendar"></i>{{ date('M d, Y', strtotime($item->created_at)) }}</a></span>
								  <span><a href="#"><i class="fa fa-map-marker-alt"></i> {{ $item->city }}</a></span>
							   </div>
							   <h3 class="postbox__title">
								  <a href="#"> {{ $item->english_title }}</a>
							   </h3>
							   <div class="postbox__text">
								  <?php $string = strip_tags($item->description);
if (strlen($item->description) > 400) {

    // truncate string
    $stringCut = substr($item->description, 0, 400);
...
echo $string; ?>
							   ...
							   <div class="postbox__read-more">
								  <a href="{{ url('blog/blog-detail/'.$item->slug) }}" class="tp-btn-xl green-anim">read more</a>
							   </div>
							</div>
						 </article>
					@endforeach
						   ...
						   @foreach($recent_posts as $key => $value)
                              <div class="rc__post mb-30 d-flex align-items-center">
                                 <div class="rc__post-thumb mr-20">
                                    <a href="{{ route('blog.detail',$value->slug) }}"><img src="{{ static_asset($value->blog_image) }}" alt="" class="more108"></a>
                                 </div>
                                 <div class="rc__post-content">
                                    ...
                                    <h3 class="rc__post-title">
                                       <a href="{{ route('blog.detail',$value->slug) }}">{{ $value->english_title }}</a>
                                    </h3>
                                 </div>
                              </div>
							 @endforeach
