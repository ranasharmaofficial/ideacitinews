  ...
  if (listItem) {
    ...
    if (siblingUL) {
      ...
      if (html.getAttribute('dir') == 'rtl') {
        if ((siblingULRect.left - siblingULRect.width - outterUlWidth < 0 && outterUlWidth < window.innerWidth) && (outterUlWidth + siblingULRect.width + siblingULRect.width < window.innerWidth)) {
          !target.classList.contains('child1') && target.classList.add('force-left')
        } else {
          !target.classList.contains('child1') && target.classList.remove('force-left')
        }
      } else {
...
(() => {
  ...
  if (
    !localStorage.getItem("aziralayout") &&
    !localStorage.getItem("aziranavstyles") &&
    !localStorage.getItem("aziraverticalstyles") &&
      ...
      switch (verticalStyle) {
        ...
        case "overlay":
          ...
          if (html.getAttribute("data-toggled") === "icon-overlay-close") {
            ...
            if (window.innerWidth >= 992) {
              if (!localStorage.getItem("aziralayout")) {
...
function menuhoverFn() {
  ...
  hasSub.forEach(e => {
    e.addEventListener('mouseover', HoverToggleInnerMenuFn)
  })
  toggleSidemenu();
  if (html.getAttribute("data-toggled") === "menu-hover-closed") {
    clearNavDropdown();
...
function checkHoriMenu() {
  ...
  if (document.querySelector("html").getAttribute("data-nav-layout") === "horizontal" && window.innerWidth > 992) {
    ...
    if (leftForceItem) {
      if (document.querySelector("html").getAttribute('dir') != 'rtl') {
        let check = leftForceItem.parentElement.closest('.slide-menu').clientWidth + leftForceItem.getBoundingClientRect().right + leftForceItem.getBoundingClientRect().width;
        if (check < innerWidth) {
          leftForceItem.classList.remove('force-left');
        }
      } else {
        let check = leftForceItem.getBoundingClientRect().left - leftForceItem.parentElement.closest('.slide-menu').clientWidth - leftForceItem.getBoundingClientRect().width;
        if (check > 0) {
          leftForceItem.classList.remove('force-left');
        }
      }
